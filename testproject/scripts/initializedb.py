...
def main(argv=sys.argv):
    ...
    with transaction.manager:
        cat1 = Category(id=1, name=u'Мысли вслух')
        cat2 = Category(id=2, name=u'Взгляд изнутри')
        cat3 = Category(id=3, name=u'Стихи')
        cat4 = Category(id=4, name=u'Программирование')
        page1 = Page(
            nick='about-me',
            title=u'Обо мне',
            content=u'''<p>Меня зовут Русанов (в прошлом Дударев) Евгений; на просторах интернета - dudev и rusevg. Занимаюсь созданием сайтов и программированием более 5 лет, организовываю концерты и прочие ивенты. Из увлечений могу отметить спорт.</p><p>В сайтостроении и программировании хорошо знаю:</p><ul>
...
<li>Assembler</li><li>C++</li><li>C#</li><li>Java</li><li>Python</li></ul><p>Разбираюсь в построении высоконагруженных систем.&nbsp;Имею понятие о многих других технологиях и языках.</p><p>Мои работы можно посмотреть в записях относящихся к разделу <a href="/blog/cat/5">Портфолио</a>.</p>'''
        ...
        page2 = Page(
            nick='offers',
            title=u'Услуги',
            content=u'''<p>Предоставлю Вам следующие услуги:</p><ul>
...
<p>
	<a href="http://dudev.ru/blog/57_Primyer-ispolzovaniya-klassa-avtopublikacii-postov-vkontaktye.html">Пример использования</a>
        ...
        comment1 = Comment(
            content = u'''В переменной $photo путь к фотографии содержится?''',
            email = 'fastyrus@mail.ru',
            author = u'Руслан',
            post_id = 20
        )
        comment2 = Comment(
            content = u'''Добрый день, Руслан.<br>
<br>
Если вы про параметр метода <i>post</i>, то нет. Там содержится <b>ID</b> фотографии. <b>ID</b> фотографии возвращает метод загрузки фотографии в альбом <i>upload_photo</i>.''',
        ...
        comment3 = Comment(
            content = u'''Спасибо. У вас фотография сначала загружается в специальный альбом методом upload_photo, который и возвращает id фотографии и путь до нее. А если нужно только фотографию кидать на стену к добавляемой записи, тогда в альбом же ее грузить не нужно?''',
            email = 'fastyrus@mail.ru',
            author = u'Руслан',
            post_id = 20
        )
        comment4 = Comment(
            content = u'''При публикации вручную фотографии прямо на стену, она сохраняется в специальный альбом. Можно ли туда загружать фотографии через API не знаю. Надо пробовать.<br>
Но если туда не получится загрузить, то почему бы не создать для этих целей отдельный альбом?''',
            email = u'admin@dudev.ru',
            author = u'Евгений',
            post_id = 20
        )
